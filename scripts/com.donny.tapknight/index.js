...
var config = {
  ...
  scriptDir: 'scripts/com.donny.tapknight/images',
  isRunning: false,
...
function useReturn(choiceF){          //各項回授點檢查 
	...
	switch (choiceF) {
		case  1:   //主畫面書本
			rbm.keepScreenshotPartial( 655,  96, 704, 148);  //(主畫面)
			var Img1 = rbm.findImage('main_book.png', 0.95);
			rbm.releaseScreenshot();

			// if (Img1 != undefined) {rbm.log('main_book:', Img1);}
			if (Img1 != undefined) {return true;} else {return false;}

		case  2:   //物品獲得圖示點選(中間)
			rbm.keepScreenshotPartial(324, 470, 397, 544);  ///點擊獲取 324,470  394,540
			for (var k = 1; k <= 4; k++) {
				var Img = rbm.findImage('getitem_' + k + '.png', 0.92);
		...
		case  6:
			...
			var Img1 = rbm.findImage('no_AutoBoss.png', 0.85);
			rbm.releaseScreenshot();
		...
		case  9:
			rbm.keepScreenshotPartial( 300,  770, 410, 805);  //(下一個區域)
			var Img1 = rbm.findImage('nextarea.png', 0.95);
			rbm.releaseScreenshot();

			// if (Img1 != undefined) {rbm.log('nextarea:', Img1);}
			if (Img1 != undefined) {return true;} else {return false;}

		case 10:
			rbm.keepScreenshotPartial( 360,  630, 480, 680);  //(隊伍升級Lv字)
			var Img1 = rbm.findImage('teamLvup.png', 0.85);
			rbm.releaseScreenshot();

			if (Img1 != undefined) {rbm.log('teamLvup:', Img1);}
			if (Img1 != undefined) {return true;} else {return false;}

		case 11:
			rbm.keepScreenshotPartial( 145,  850, 236, 926);  //(每日獎勵)
			var Img1 = rbm.findImage('dailyreward.png', 0.95);
			rbm.releaseScreenshot();

			if (Img1 != undefined) {rbm.log('dailyreward:', Img1);}
			if (Img1 != undefined) {return true;} else {return false;}

		case 12:
			rbm.keepScreenshotPartial( 253,  392, 458, 444);  //(更新程式資料)
			var Img1 = rbm.findImage('updata.png', 0.95);
			rbm.releaseScreenshot();

			if (Img1 != undefined) {rbm.log('updata:', Img1);}
			if (Img1 != undefined) {return true;} else {return false;}

		case 13:
			rbm.keepScreenshotPartial( 253,  352, 458, 484);  //(下載雲端存檔)
			var Img1 = rbm.findImage('downloadsave.png', 0.90);
			rbm.releaseScreenshot();

			if (Img1 != undefined) {rbm.log('downloadsave:', Img1);}
			if (Img1 != undefined) {return true;} else {return false;}
...
function powerTap() {
	...
	for (var i = 1; i <= 3; i++) {
		...
		if (pwcolor) { swipFor(210, 870, 4, 10, 40, 50); }
		else break;
		// sleep(100);
	}
	return true;
...
function buyPosition(Timer) {
	...
	if (Img1 != undefined) {
		swipFor(240, 950, 5, 70, 80, 100);
		swipFor( 80, 950, 5, 70, 80, 100);
		keycode('BACK', 600);
	}
...
function debug(Timer){       //異常檢查檢查
	...
	if (useReturn(1)) {
		...
		if (useReturn(2)) {bookError = 0;}; //獲取物品(中間)
		if (useReturn(3)) {swipFor(360, 750, 1, 50, 100, 100);}	//沒廣告可看
		if (useReturn(5)) {swipFor(300, 730, 1, 50, 100, 100); bookError = 0;}  //退出選單
		if (useReturn(9)) {swipFor(288, 777, 1, 50, 100, 100); bookError = 0;}	//下一區
		if (useReturn(10)) {swipFor(370, 640, 1, 50, 100, 100); bookError = 0;}	//隊伍升級
		if (useReturn(11)) {swipFor(260, 910, 1, 50, 100, 100); bookError = 0;}	//每日獎勵
		if (useReturn(12)) {swipFor(360, 850, 1, 50, 100, 100); bookError = 0;}	//更新程式資料
		if (useReturn(13)) {swipFor(480, 830, 1, 50, 100, 100); bookError = 0;}	//下載雲端存檔

		if (bookError >= 10) {
			bookError2 = bookError2 + 1;
			console.log('沒有書 ' + bookError + ' 次，按退回');
...
function main(){       //主流程
	...
	if (mainBook) {
		// console.log('main 施放技能')
		randTapskill(4); sleep(s);   //施放技能

		// console.log('main 點擊能量');
		powerTap(); sleep(s);  //點擊能量

		// console.log('main 取定時獎勵');
		getADraward(3); sleep(s);    //取定時獎勵

		// console.log('main 商店買水');
		buyPosition(160); sleep(s);    //商店買水

		// console.log('main 清包包-魔力取出');
		clearnBag(900); sleep(s);    //商店買水

		// console.log('main 英雄升級max');
		lvmaxHero(300); sleep(s);    //商店買水
		...
		bookError = bookError + 1; 
		// console.log('不在主畫面:', bookError);

		if (bookError2 >= 4) {
			console.log('沒有書 ' + 10 * bookError2 + ' 次，重啟遊戲');
			RestartApp(20);
			bookError2 = 0;
		}
	}

	// console.log('main 任務金幣');
	getQuestgold(); sleep(s);     //任務金幣
...
function setFirstTimer() {   //預設值設定
	randTapskillTimer = Date.now() +   5 * 1000;
	getADrawardTimer  = Date.now() +   1 * 1000;
	getQuestgoldTimer = Date.now() +   5 * 1000;
	buyPositionTimer  = Date.now() +  60 * 1000;
...
// 獎勵：   5000G(30sec)→10RUBY(60sec)→10000G(60sec)→
//         20RUBY(2min)→10000G(3min)→AutoSkill(5min)→
//         30RUBY(5min)→10000G(10min)→30RUBY(10min)→

// stage:5  time:12:42  gold:20290
// stage:5  time:12:47  gold:23270
// stage:5  time:12:52  gold:26510  -> 6220
// 
// stage:10  time:12:57:33  gold:35182
// stage:10  time:13:04:30  gold:39522
// stage:10  time:13:07:30  gold:41462 -> 6360
// 
// stage:15  time:13:24:20  gold:49851
// stage:15  time:13:29:20  gold:53742
// stage:15  time:13:07:30  gold: -> 3891/5min
// 
// stage:25  time:20:58:20  gold:129993
// stage:25  time:21:03:20  gold:136500
// stage:15  time:13:07:30  gold: -> 6507/10min
// 
// 任務升級： +13%
//
// 回城水晶：
