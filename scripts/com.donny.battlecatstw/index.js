...
function goldegg(c1, mod) {   //mod: 0:lucky card  1:
	...
	for (var i = 1; i <= c1; i++) {
		...
		tapFor(980, 600, 55, 100, 100, 1000);
...
function clearnStore(mod) {  //mod: 0:不換np/xp  1:換np  2:換xp
	...
	// sleep(1000);
	tapFor(180, 600, 2, 100, 100, 1500); //點儲藏庫

	tapFor(440, 680, 2, 100, 100, 1000); //點全部使用
	tapFor(320, 440, 2, 100, 100, 1000); //全部使用-點是
	tapFor(320, 440, 2, 100, 100, 1000); //確定使用-點是
	tapFor(840, 440, 2, 100, 100, 1000); //已經使用-點OK
	...
	if (mod != 0) {
		tapFor(740, 680, 2, 100, 100, 1500); //點換 NP/XP
		
		if (mod == 1) {
			tapFor(320, 440, 2, 100, 100, 1000); //一併交換-換成NP
		} else if(mod == 2){
			tapFor(800, 440, 2, 100, 100, 1000); //一併交換-換成XP
		}

		tapFor(320, 440, 2, 100, 100, 1000); //確定換成-點是
	}
	

	tapFor( 60, 680, 2, 100, 100, 1000); //點返回
}
